...
- [x] **B-17 [HIGH]** DONE 2026-08-07 — `~/.claude.json|availability|global-fallback-points-at-a-live-worktree`. The machine-wide `codesift` MCP entry ran `node /Users/greglas/DEV/codesift-mcp/dist/server.js`, and `npm run build` opens with `rmSync('dist')`. So **every build of this repo broke codesift for every NEW agent session on the machine** that had no per-project entry — and freshly created worktrees never do. Measured in `~/.zuvo/retros.log` (field 16 reports codesift availability per run): 2026-08-03 had 2 `unavailable` out of 90; **2026-08-07 had 13 out of 19 (68%)**, clustered 00:27-01:32 and 08:58-09:14. 2026-08-04 additionally shows 13 `transport_closed`, the day 78 projects were switched to the HTTP daemon. Fixed by pointing the fallback at the stable global install (`~/.npm-global/bin/codesift-mcp`, 0.14.0), which a build cannot delete; the ollama env is preserved. NOT proven that this caused each of the 13 — the structural exposure is proven, the per-incident attribution is not. Remaining coupling: the supervised daemon still executes `dist/cli.js` from the working tree (mitigated by the build-restart hook from B-16, but not removed). Running it from the global install too would close it, at the cost of local changes no longer reaching the daemon without `npm i -g` — a real dev-loop trade, so it is a decision rather than a cleanup.
