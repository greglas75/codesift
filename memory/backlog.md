...
- [ ] **B-16 [MED]** `package.json|operational|build-nukes-the-running-daemon` — `npm run build` starts with `rmSync('dist',{recursive:true,force:true})`, and the launchd daemon (`com.codesift.daemon`) executes `dist/cli.js`. Building the repo therefore deletes the code a machine-wide service is running from. Node has already resolved the modules it loaded at startup, so the daemon keeps answering `/health` with 200 — it only breaks later, on the first LAZILY imported module, with a message that looks like a source bug rather than an operational one: `The requested module './index-tools.js' does not provide an export named 'getIndexSummary'` (the export exists in both src and dist; the module graph was stale). Reproduced 2026-08-04: every tool call through the daemon failed this way after a routine build, while the health endpoint stayed green. Now that 78 Claude Code projects route through the daemon (B-15) this hits every repo at once, and `/health` will not show it. Recipe: (1) have `npm run build` emit to a temp dir and swap, or at minimum kickstart the LaunchAgent afterwards; (2) make `/health` exercise one lazily-imported module so a poisoned graph is visible; (3) consider having the daemon run from an installed copy rather than the working tree's dist. Workaround today: `launchctl kickstart -k gui/$(id -u)/com.codesift.daemon`.
