...
- [ ] [LOW] `src/storage/sqlite-index-store.ts|consistency|rethrowOperational-coverage` — only openIndexDb and loadIndexSqlite wrap raw sqlite errors as IndexStorageError; the narrow accessors and write paths let them propagate raw, so a caller using isIndexStorageError (rather than classifyStorageError) will not recognise them. Defer reason: [structural-refactor (multi-file)].
