 ...
 */
async function renderSymbol(sym: CodeSymbol, force?: boolean): Promise<string> {
  const { symbol, note } = elideShownSource(sym, force ? { force } : undefined);
  const text = await formatSymbolCompact(symbol);
  return note ? `${text}\n${note}` : text;
}
...
export const CORE_SYMBOL_TOOL_ENTRIES: ToolDefinitionEntry[] = [
  ...
  { order: 1454, definition: {
    ...
    handler: async (args) => {
      ...
      let text = resolvedNote + await renderSymbol(result.symbol, args.full_source as boolean | undefined);
  ...
  { order: 1489, definition: {
    ...
    handler: async (args) => {
      ...
      const output = (await Promise.all(syms.map((sym) => renderSymbol(sym, force)))).join("\n\n");
  ...
  { order: 1528, definition: {
    ...
    handler: async (args) => {
      ...
      let text = formatIdAmbiguity(result.id_ambiguity) + (await renderSymbol(result.symbol));
  ...
  { order: 1548, definition: {
    ...
    handler: async (args) => {
      ...
      const { symbol, note } = elideShownSource(bundle.symbol as CodeSymbol);
      let text = await formatBundleCompact({ ...bundle, symbol });
      // The symbol header is the bundle's first line; the pointer belongs directly under it.
      if (note) text = text.replace(/^[^\n]*/, (header) => `${header}\n${note}`);
      return formatIdAmbiguity(bundle.id_ambiguity) + text;
    },
  } },
