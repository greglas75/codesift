...
## Which languages actually have symbols — and the two ways agents get this wrong (2026-09-18)

Both failure modes below were observed in one external agent run on a Kotlin/Android repo. Neither
is a tool fault: in both cases CodeSift reported the truth and the agent drew the opposite
conclusion from documentation that was stale or silent. Both end the same way — the agent announces
it is "skipping CodeSift" and falls back to reading files by hand, which is the expensive path this
whole server exists to remove.

**"Kotlin is only a stub" is false.** The authoritative source is `EXTENSION_LANGUAGE_MAP` in
`parser-manager.ts` plus the `switch` in `symbol-extractor.ts`, not any prose:

| tier | languages | what you get |
|---|---|---|
| tree-sitter grammar + dedicated extractor | TypeScript/TSX, JavaScript, Python, Go, Rust, **Kotlin** (`.kt`/`.kts`), Gradle KTS (`.gradle.kts`), PHP | full symbols, call graph, references, `analyze_complexity`, language tools |
| grammar, generic extractor | Java, Ruby, CSS | symbols via `extractGenericSymbols` |
| regex / custom extractor, no grammar | Astro, Prisma, Markdown, SQL, Hono model, conversations | symbols, narrower |
| `text_stub` / `config` — **no symbol extraction** | swift, dart, scala, clojure, elixir, lua, zig, nim, `.gradle`, sbt, html, json/yaml/toml/ini/env | file is indexed: `get_file_tree`, `search_text`, `scan_secrets` work; `search_symbols` does not |

Kotlin additionally has ~10 dedicated tools (Compose tree + recomposition, Room, Hilt, Flow chain,
KMP, Kotest, 6 anti-patterns) that auto-enable on `build.gradle.kts`. **Never infer the tier from a
heading, a rules file, or this table alone — `index_status` returns the per-repo language breakdown
and the `text_stub` list for the repo in front of you.** That is the check, and it costs one call.

**A stale index is a reindex trigger, not a reason to skip CodeSift.** `index_status` reports
`commit.indexed`, `commit.head`, `commit.matches` and `commit.files_changed` precisely so an agent
can tell "index is from another commit" from "index cannot be established" (`e6b1d33`). Reading
`files_changed: 235` as "CodeSift is unusable here" inverts the intent: the fix is
`index_folder(path=<root>)` once — seconds to minutes — after which every answer is current. Falling
back to `grep` instead costs more, on every subsequent question, and gives up the call graph
entirely. The one case where an old index is genuinely unusable is a **linked worktree**, and there
the fix is also `index_folder`, not a fallback (see ALWAYS/NEVER in `rules/codesift.md`).

**`analyze_complexity` IS regex over each symbol's source**, in every language — not AST, not
Kotlin-specific (`complexity-tools.ts:49`, language-gated pattern sets since 2026-07-30). Reporting
that as a Kotlin caveat implies the other languages are AST-scored; they are not. It is a ranking
signal, and the docs should keep calling it that.
