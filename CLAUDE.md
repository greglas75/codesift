...
Non-core tools are **hidden** from ListTools (via SDK `disable()`). Only 48 core tools are visible (out of 161 total).
...
**src/search/** (6 files) — BM25F index with centrality bonus, semantic embeddings, chunker; tool-ranker.ts (5-signal ranking: BM25, identity, semantic, usage-freq, framework)
