...
describe("indexFolder persistent snapshot diff", () => {
  ...
  it("(g) DROP-guard rejection preserves the old snapshot", async () => {
    ...
    // Force a partial walk: scope to a subdir that contains a single file —
    // far below 50% of the 60-file baseline → triggers rejected_partial.
    // The existing files still exist on disk so auto-heal does NOT kick in.
    await mkdir(join(fixtureDir, "tiny"), { recursive: true });
    await writeFile(join(fixtureDir, "tiny", "only.ts"), `export const only = 1;\n`);

    resetIndexFolderRedundancyForTesting();
    const r2 = await indexFolder(fixtureDir, { include_paths: ["tiny"], watch: false });
