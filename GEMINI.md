...
**IMPORTANT (Gemini/Antigravity):** The MCP server may not see the correct working directory.
At the start of each session, call `list_repos()` to find the repo name for your project.
Pass the `repo` parameter explicitly to all tool calls (e.g. `repo="local/my-project"`).
If the repo is not yet indexed, run `index_folder(path="/full/path/to/project")` once.
...
## Language coverage — verify with `index_status`, never assume

- **Full symbols**: TS/TSX, JS, Python, Go, Rust, **Kotlin** (`.kt`/`.kts`/`.gradle.kts`), PHP.
  Kotlin is NOT a stub — it also has ~10 dedicated tools (Compose, Room, Hilt, Flow, KMP, Kotest).
  Generic extractor: Java, Ruby, CSS. Custom/regex: Astro, Prisma, Markdown, SQL.
- **`text_stub` = no symbols**: swift, dart, scala, clojure, elixir, lua, zig, nim, `.gradle`, sbt,
  html, config. Those files ARE indexed — `search_text`, `get_file_tree`, `scan_secrets` work on
  them; only `search_symbols` and the graph tools do not.

## A stale index means reindex — never "skip CodeSift"

`commit.matches=false` / `files_changed=N` from `index_status` is an instruction to run
`index_folder(path=<root>)` ONCE, not evidence that the server is unusable. Falling back to
`grep`/`find` pays more on every later question and gives up the call graph, references and
complexity entirely. Same answer in a linked worktree: index it, do not fall back.

`analyze_complexity` is regex over symbol source in EVERY language — a ranking signal, not a
quality gate, and not a per-language caveat.
...
- Skip indexing a worktree you are actually working in. A linked worktree has the main
  checkout as a path ancestor, so an unindexed one resolves to the PARENT repo and you get
  that checkout's files. `index_folder(path=<worktree>)` makes it resolve to itself; hint
  `H19` fires whenever an answer describes a different working tree than your CWD
